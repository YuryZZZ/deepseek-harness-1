...
flowchart LR
  ...
  pkg_langfuse_ingest["langfuse-ingest"]
  pkg_langfuse["langfuse"]
  svc_langfuse["ctx.langfuse<br/>Langfuse observability client and analysis"]
  pkg_tool_langfuse["tool-langfuse"]
  pkg_storage["storage"]
  ...
  pkg_langfuse --> svc_langfuse
  pkg_langfuse_ingest --> svc_sessionTelemetry
  pkg_llm --> svc_llm
  ...
  svc_langfuse --> pkg_langfuse_ingest
  svc_langfuse --> pkg_tool_langfuse
...
| `ctx.sessionTelemetry` | `seam` | [`session-telemetry`](../packages/session/session-telemetry) | [`session-telemetry-otel`](../packages/session/session-telemetry-otel), [`langfuse-ingest`](../packages/langfuse/langfuse-ingest) | - | - | The seam captures, redacts, and hands session records to one backend; nothing else consumes the service — its output leaves the process. |
| `ctx.langfuse` | `core` | [`langfuse`](../packages/langfuse/langfuse) | - | [`tool-langfuse`](../packages/langfuse/tool-langfuse), [`langfuse-ingest`](../packages/langfuse/langfuse-ingest) | - | Persistent client over a Langfuse server: reads traces, observations, scores, and metrics, analyzes them into insights and recommendations, and writes scores and ingestion events back. |
