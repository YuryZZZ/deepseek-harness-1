...
English | [中文](langfuse.zh.md)

The `ctx.langfuse` service is a persistent client over a self-hosted or cloud Langfuse server plus a pure analysis engine that reduces raw telemetry — traces, observations, scores, and metrics — into prioritized insights, recommendations, and research directions. See [`@deepseek-ai/dsh-langfuse`](../../packages/langfuse/langfuse/README.md) for the service contract and [`@deepseek-ai/dsh-tool-langfuse`](../../packages/langfuse/tool-langfuse/README.md) for the model-facing Consumer.
